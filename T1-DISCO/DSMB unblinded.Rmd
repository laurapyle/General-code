...
user <- Sys.info()[["user"]]
if (user == "laurapyle") {
  data_path <- "/Users/laurapyle/Library/Application Support/Mountain Duck/Volumes.noindex/Kopah S3.localized/core.data/clinical/t1disco"
  rand_path <- "/Users/laurapyle/Library/CloudStorage/OneDrive-UW/T1-DISCO randomization"
} else if (user == "lpyle") {
  data_path <- '/Users/lpyle/Library/Application Support/Mountain Duck/Volumes.noindex/Kopah S3.localized/core.data/clinical/t1disco'
  rand_path <- "/Users/lpyle/Library/CloudStorage/OneDrive-UW/T1-DISCO randomization"
} else if (user == "pylell") {
  data_path <- '/Users/pylell/Library/Application Support/Mountain Duck/Volumes.noindex/Kopah S3.localized/core.data/clinical/t1disco'
...
filename <- "T1DISCO randomization for DSMB.xlsx"
full_path <- file.path(rand_path, filename)
rand <- read_xlsx(full_path, sheet = "Sheet1") %>% arrange(ID)
rand_check <- read_xlsx(full_path, sheet = "Sheet2") %>% arrange(ID)
...
# read in lab data
filename <- "T1DISCO-SafetyLabs_DATA_LABELS_2026-07-07_1714.xlsx"
full_path <- file.path(data_path, filename)
labs <- read_xlsx(full_path, na = c("-", "", " ", "-9999", "-1"))
#labs <- read.csv(full_path, na = c("-", "", " ", "-9999", "-1"))
# fix a visit type
labs$`Visit Type` <- ifelse(labs$`Record ID` == "DSCO-118" & labs$`Event Name` == "Visit 4 - 2 month", "Month 2", labs$`Visit Type`)
...
# recode group consistently
labs$Group <- str_to_upper(labs$Group)
labs$Group <- ifelse(labs$Group == "ACTIVE", "SEMAGLUTIDE", labs$Group)

# fix a bicarb value
labs$`Bicarb (mmol/L)` <- ifelse(labs$`Bicarb (mmol/L)` == 252, 25, labs$`Bicarb (mmol/L)`)
# convert biliribun and fix one value
labs$`Total Bilirubin (mg/dl)` <- ifelse(labs$`Total Bilirubin (mg/dl)` == "05", "0.5", labs$`Total Bilirubin (mg/dl)`)
labs$`Total Bilirubin (mg/dl)` <- as.numeric(labs$`Total Bilirubin (mg/dl)`)


# read in CGM data
filename <- "T1DISCO-CGM_DATA_LABELS_2026-07-07_1652.xlsx"
full_path <- file.path(data_path, filename)
...
# recode group consistently
cgm_summary$Group <- str_to_upper(cgm_summary$Group)
cgm_summary$Group <- ifelse(cgm_summary$Group == "ACTIVE", "SEMAGLUTIDE", cgm_summary$Group)
