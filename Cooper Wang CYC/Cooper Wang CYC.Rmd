...
```{r echo=FALSE, comment="", message=FALSE}
mod <-  geeglm(CRR ~ dose*time + percent_expected_dose_div_10 + age_cyc1_yrs + diagnosis_GFR10 + upc_ratio + 
                 cyclophosphamide_use_2 + duration_sle_to_cyc1 + gender + race5 + 
                 Prednisone + IVsteroids_div_1000 + ACEARB + Ritux, 
               id=record_id, data=data, family="binomial", scale.fix=TRUE, corstr="ar1")
#anova(mod)
t <- mod %>% tbl_regression(family="binomial", exponentiate = TRUE) %>% add_global_p() 
gtsummary::modify_table_body(t, dplyr::select, -p.value)
test.emm <- emmeans(mod, ~ dose*time, type="response")
p <- as.data.frame(pairs(test.emm, adjust="none"))
p <- p[p$contrast=="High (NIH) time12 / (Low (euro-lupus) time12)",]
p$df <- NULL
p$z.ratio <- NULL
p

p <- as.data.frame(confint(pairs(test.emm), type = "response", adjust="none"))
p <- p[p$contrast=="High (NIH) time12 / (Low (euro-lupus) time12)",]
p$df <- NULL
p$z.ratio <- NULL
p
...
```{r echo=FALSE, comment="", message=FALSE}
mod <-  geeglm(CRRPRR ~ dose*time + percent_expected_dose_div_10 + age_cyc1_yrs + diagnosis_GFR10 + upc_ratio + 
                 cyclophosphamide_use_2 + duration_sle_to_cyc1 + gender + race5 + 
                 Prednisone + IVsteroids_div_1000 + ACEARB + Ritux, 
               id=record_id, data=data, family=binomial, scale.fix=TRUE, corstr="ar1")
#anova(mod)
