...
# create a table summarizing the descriptive statistics
...
t2 <- left_join(t2, cs, by = c("site_long", "season"))

# need to figure out why NA in contraindication
```
...
4) To exclude hospitalizations not between the first and last date of screening in a season, I excluded based on the admission date only. Would it be better to require that the discharge date was before the last screening date?
