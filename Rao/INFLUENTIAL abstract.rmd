...
check_vaccination_dates <- function(data) {
  ...
  for (i in 1:nrow(data)) {
    ...
    for (col in vaxx_cols) {
      ...
      data$vaccine_this_season[i] <- ifelse(vaxx_date >= first_screen & vaxx_date <= last_screen & vaxx_date < adm_date, 1, 0)
      data$vaccine_during_hosp[i] <- ifelse(vaxx_date >= first_screen & vaxx_date <= last_screen & vaxx_date >= adm_date 
          && vaxx_date <= dish_date, 1, 0)
