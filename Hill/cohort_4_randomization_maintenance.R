...
participants <- read.csv('/Users/pylell/Library/CloudStorage/OneDrive-UW/Hill/23-2151 Cohort 4 Randomization with Assignments UPDATE.csv')

# Participants lost to follow-up - delete
participants <- participants %>% filter(!Assigned.Participant.ID == "Bfed252")
participants <- participants %>% filter(!Assigned.Participant.ID == "Bfed255")
participants <- participants %>% filter(!Assigned.Participant.ID == "Bfed257")
participants$What.was.your.biological.sex.assigned.at.birth. <- ifelse(participants$What.was.your.biological.sex.assigned.at.birth. == "Male ",
                                                                       "Male", participants$What.was.your.biological.sex.assigned.at.birth.)
...
table(rand$initial_intervention, rand$treatment_char)

# output
write.csv(rand, 
          "/Users/pylell/Library/CloudStorage/OneDrive-UW/Hill/23-2151 Cohort 4 Randomization with Assignments UPDATE with Maintenance Assignments.csv",
          row.names = F)
