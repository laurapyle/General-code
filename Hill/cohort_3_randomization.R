...
participants <- read.csv('/Volumes/pylell/Hill/Randomization/23-2151 Cohort 3 Randomization.csv')
participants$X <- NULL
participants$X.1 <- NULL

# stratify by biological sex and randomize to counseling or no counseling
males <- participants %>% filter(What.was.your.biological.sex.assigned.at.birth. %in% c("Male", "Male "))
...
# output
write.csv(rand, 
          "/Volumes/pylell/Hill/Randomization/23-2151 Cohort 3 Randomization with Assignments.csv",
          row.names = F)
