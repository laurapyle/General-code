...
participants <- read.csv('/Volumes/pylell/Hill/Randomization/Maintenance phase randomization/23-2151 Cohort 3 Randomization with Assignments UPDATE.csv')

# Participants lost to follow-up - delete
participants <- participants %>% filter(!Assigned.Participant.ID == "Bfed237")
participants <- participants %>% filter(!Assigned.Participant.ID == "Bfed239")
...
# output
write.csv(rand, 
          "/Volumes/pylell/Hill/Randomization/Maintenance phase randomization/23-2151 Cohort 3 Randomization with Maintenance Assignments.csv",
          row.names = F)
